...
class ParametersPresenter extends BasePresenter{
	...
	public function actionUpdateParameter($idPage, $id){
		...
		if($id) $this->parameter = $this->repository->find($id);
		else $this->parameter = new \WebCMS\EshopModule\Doctrine\Parameter;
	...
	public function actionUpdateValue($id, $idPage, $parameter){
		...
		if($id) $this->value = $this->valueRepository->find($id);
		else $this->value = new \WebCMS\EshopModule\Doctrine\ParameterValue;
		
		if($parameter) $this->parameter = $this->repository->find($parameter);
		else $this->parameter = new \WebCMS\EshopModule\Doctrine\Parameter;
