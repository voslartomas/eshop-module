...
use Nette\Application\UI;
	...
	public function productFormSubmitted(UI\Form $form) {
	    ...
	    $this->product->setAvailability($values->availability);
	...
	protected function createComponentProductsGrid($name) {
	    ...
	    $grid = $this->createGrid($this, $name, '\WebCMS\EshopModule\Doctrine\Product', NULL, array(
		'language = ' . $this->state->language->getId(),
		)
	    );
	...
	public function actionUpdateProduct($idPage, $id) {
		...
		$this->product = new \WebCMS\EshopModule\Doctrine\Product();
	...
	public function actionUpdateVariant($idPage, $idProduct, $id) {
	    if ($id)
		$this->variant = $this->repository->find($id);
	    else
		$this->variant = new \WebCMS\EshopModule\Doctrine\Product;

	    if ($idProduct)
		$this->product = $this->repository->find($idProduct);
	    else
		$this->product = new \WebCMS\EshopModule\Doctrine\Product();
	}
	...
	public function createComponentParameterForm() {
	    ...
	    $parameters = $this->em->getRepository('\WebCMS\EshopModule\Doctrine\Parameter')->findAll();

	    foreach($parameters as $p){
		$form->addGroup($p->getName());

		foreach($p->getValues() as $v){
		    if(in_array($v->getId(), $this->product->getParametersValuesId())){
			$form->addCheckbox('value' . $v->getId(), $v->getValue())->setValue(true);
		    }else{
			$form->addCheckbox('value' . $v->getId(), $v->getValue());
		    }
		}
	    }
	    
	    $form->addHidden('idProduct', $this->product->getId());
	    $form->addSubmit('save', 'Save')->setAttribute('class', 'btn btn-success');
	...
	public function parameterFormSubmitted($form) {
		...
		$parameter = $this->em->getRepository('\WebCMS\EshopModule\Doctrine\ParameterValue')->find($id);
		if($v){
	...
	public function actionUpdateParameter($idParameter, $idProduct, $idPage) {
	    
	    $this->product = $this->repository->find($idProduct);
	    
	    if(is_numeric($idParameter)){
		$this->parameter = $this->product->getParameterByIdValue($idParameter);
	    }
	}

	public function renderUpdateParameter() {
	    $this->reloadModalContent();
	}

	public function actionDeleteParameter($id, $idProduct) {
